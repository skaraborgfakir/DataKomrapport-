%%
%% Time-stamp: <2019-05-29 17:53:37 stefan>
%%

\documentclass[draft,swedish,10pt,a4paper]{article}
...
\hypersetup{%
  ...
  pdfkeywords={%
    Datakommunikation, IEEE802.3, DNS, IP, UDP, TCP, ARP, RARP
  }
}
...
XEROX Alto och Star (kommersialiseringen av Alto som i huvudsak var prototyper och utvecklingssystem
...
med avsikten att han skulle forska inom XEROX kontorsautomatiseringsprojekt\footcite[kap 6 sekt 7]{Pelkey}.

Metcalfes doktorsavhandling som var baserad på hans arbete inom Arpanet-projektet refuserades vid disputationen
våren 1972 med motivationen att dess matematiska innehåll var otillräckligt och att det saknas konkreta teorier
vilket tvingade honom att hitta nya vinklar och förbättra matematiken. Norman Abramsons text\footcite{AbramsonAloha}
hade han läst tidigare under året men han ansåg då vid läsningen att texten var felaktig vad
beträffar modelleringen av hur människor reagerar när en förbindelse har
större och märkbara fördröjningar. Modellen i Abramsons text antog som en förutsättning att människor fortsätter
(specifikt) att mata in text även när svaren från värddatorn är kraftigt fördröjda.

Han frågade XEROX om lov att resa till Hawaii och Norman Abramson om det var möjligt
att få komma som gästforskare och fick bifall från båda hållen.
...
\section{DNS Konfigurationsfiler}\label{sec:appendix_dns_conf_files}
...
\section{isc dhcp kontrollantens konfiguration}\label{sec:appendix_isc_dhcpd_config}
...
\section{dhcp klienters lease}\label{sec:appendix_dhcp_leases}
